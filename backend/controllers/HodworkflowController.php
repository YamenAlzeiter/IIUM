...
class HodworkflowController extends \yii\web\Controller
{
    public $layout = 'blank';
    ...
    public function actionHodApproval($ID)
    {
        ...
        $modelPoc = new Poc();
        return $this->render('hodApproval', ['model' => $this->findModel($ID), 'modelKedio' => $modelKedio, 'modelPoc' => $modelPoc,]);
    }
    ...
    public function  actionReject($ID){
        ...
        if ($this->request->isPost) {
            ...
            Yii::debug('Token after updating: ' . $model->token);
            if ($model->save()) {
//reroute new view application has been submitted, retrive the name
            }
        }}
}
